 ...
 * @param error the [com.nerdery.rtaza.jobtracker.data.core.Error] that the Resource wraps in the case that the Resource
 * has a status of [Status.ERROR].
 */
sealed class Resource<DataType>(
    val status: Status,
    val data: DataType?,
    val error: com.nerdery.rtaza.jobtracker.data.core.Error?
) {
    ...
    class Error<DataType>(error: com.nerdery.rtaza.jobtracker.data.core.Error) :
